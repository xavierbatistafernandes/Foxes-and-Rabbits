...
void solve_conflicts_and_breeding(Cell *curr_world, Cell *next_world, int *updated, int c1, int i2, int j2, int breed_age_f, int breed_age_r, int local_cols){
    ...
    //next_world[c1].breed_age = -1;        
    //next_world[c1].starve_age = -1;
...
int main(int argc, char *argv[]){
    ...
    int i = 0, j = 0, k, c;                     /*Iteration variables*/
    ...
    int global_row, global_col;                 /*Global row and global column index*/
    ...
    for(k = 1; k <= generations; k++){
        ...
        for(i = 2; i < local_rows-2; i++){           
            ...
            for(j = (global_row + first_col_proc)%2 + 2; j < local_cols-2; j+=2){   /*Process only the black cells*/
                ...
                c = i*local_cols + j;
                if(updated[c] == 1) {                   /*If an animal has moved to a black cell from a red cell, then update is set to 1 in that position, meaning in this loop it is skipped*/
                    continue;
                }
                if(curr_world[c].animal == FOX || curr_world[c].animal == RABBIT)    /*Only try to move if the cell has a fox or rabbit*/
                    movement(curr_world, next_world, updated, local_rows, local_cols, \
